...
class Video extends CI_Controller {
	...
	*/
	...
	public function transcode(){
		...
		$getvideotask = $this->db->query("SELECT * FROM `".CS_SqlPrefix."queue` WHERE `vid` = ".$id." ORDER BY `mu_queue`.`xu` ASC limit 1")->row();
		if(empty($getvideotask) && $getvideotask == NULL){
		    //原程序逻辑有问题，我懒得改结构，直接互换一下vid和id
		    //修改时间 2022.10.31
		    //New_Paper
		}else{
		    $id = $getvideotask->id;
