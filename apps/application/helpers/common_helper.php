...
function m3u8_link($path,$ac='m3u8',$xu=1){
	...
	if(Web_Play == ''){
		if(Web_M3u8On == 1){
			if($ac == 'm3u8'){
				$link = 'https://'.Web_Url.$webpath.'m3u8/'.sys_auth($path).'.'.$ac;
			}else{
				$link = 'https://'.Web_Url.$webpath.'m3u8/pic/'.sys_auth($path).'.'.$ac;	
			}
		}else{
			$link = 'https://'.Web_Url.Web_Path.$Mu_Path.$path;
		}
	}else{
		if(Web_M3u8On == 1){
			if($ac == 'm3u8'){
				$link = 'https://'.Web_Play.$webpath.'m3u8/'.sys_auth($path).'.'.$ac;
			}else{
				$link = 'https://'.Web_Play.$webpath.'m3u8/pic/'.sys_auth($path).'.'.$ac;
			}
		}else{
			$link = 'https://'.Web_Play.'/'.$path;
		}
	}
	return $link;
}
